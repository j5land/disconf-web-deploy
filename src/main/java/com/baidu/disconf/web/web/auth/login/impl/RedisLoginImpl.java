...
public class RedisLoginImpl implements RedisLogin {
    ...
    public Visitor isLogin(HttpServletRequest request) {
        ...
        if (xId != null) {
            ...
            Visitor visitor = (Visitor)this.jedisCluster.getObj(ModuleKey.SP, "user", "token");
    ...
    private void updateRedisVisitor(Visitor visitor, HttpServletRequest request, int expireTime) {
        ...
        if (xcookieName != null) {
            ...
            // 更新
            if (visitor != null) {
            	this.jedisCluster.setObj(ModuleKey.SP, "user", "token", visitor);
            } else {
            	this.jedisCluster.del(ModuleKey.SP + ":user:token");
                // 删除
            }
        }
    }
